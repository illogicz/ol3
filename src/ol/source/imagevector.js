...
ol.source.ImageVector.prototype.canvasFunctionInternal_ = function(extent, resolution, pixelRatio, size, projection) {
  ...
  var loading = false;
  this.source_.forEachFeatureInExtent(renderExtent,
      /**
  ...
  if (!this.source_.getWrapX()) {
    ...
    if (extent[1] < sourceExtent[3] && extent[3] > sourceExtent[1]) {
      var worldWidth = ol.extent.getWidth(projectionExtent);
      var offsetX = 0;
      while (extent[2] - offsetX - worldWidth > sourceExtent[0]) {
        offsetX += worldWidth;
      }
      while (extent[2] - offsetX < sourceExtent[0]) {
        offsetX -= worldWidth;
      }
